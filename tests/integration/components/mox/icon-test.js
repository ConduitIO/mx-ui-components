...
module('Integration | Component | mox/icon', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.dom('[data-test-mox-icon="connectors-16"] use').hasAttribute('xlink:href', '/svg-defs.svg#connectors-16');
  });
  ...
  test('it renders with currentFill and currentStroke by default', async function(assert) {
    ...
    assert.dom('[data-test-mox-icon="connectors-16"]').hasClass('stroke-current');
  });

  test('it renders (@noFill)', async function(assert) {
    await render(hbs`<Mox::Icon @iconName="connectors-16" @noFill={{true}} />`);

    assert.dom('[data-test-mox-icon="connectors-16"]').doesNotHaveClass('fill-current');
    assert.dom('[data-test-mox-icon="connectors-16"]').hasClass('stroke-current');
  });

  test('it renders (@noStroke)', async function(assert) {
    await render(hbs`<Mox::Icon @iconName="connectors-16" @noStroke={{true}} />`);

    assert.dom('[data-test-mox-icon="connectors-16"]').hasClass('fill-current');
    assert.dom('[data-test-mox-icon="connectors-16"]').doesNotHaveClass('stroke-current');
  });
});
