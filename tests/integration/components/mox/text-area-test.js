...
module('Integration | Component | mox/text-area', function (hooks) {
  ...
  test('the disabled input state is accessible', async function (assert) {
    ...
    await render(
      hbs`<Mox::TextArea @onInput={{this.onInput}} @label="Address" @id="address-field" disabled />`
    );
    await a11yAudit();
  ...
  test('the invalid input state is accessible', async function (assert) {
    ...
    await render(
      hbs`<div class="bg-gray-900"><Mox::TextArea @onInput={{this.onInput}} @label="Address" @id="address-field" @isValid={{false}} @error="Missing something?" /></div>`
    );
    await a11yAudit();
    assert.ok(true, 'no accessibility errors');
  });
