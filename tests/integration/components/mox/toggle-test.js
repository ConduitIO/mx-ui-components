...
module('Integration | Component | mox/toggle', function(hooks) {
  ...
  test('it displays the label', async function(assert) {
    await render(hbs`<Mox::Toggle @toggleAction={{this.toggleAction}} @label={{this.label}} />`);
  ...
  test('it throws an error if it is not configured in an accessible way', async function(assert) {
    ...
    setupOnerror((error) => {
      ...
      assert.ok(error.message.includes(`To render an accessible checkbox element, you must pass a @label argument`));
    });
