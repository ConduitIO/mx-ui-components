...
module('Integration | Component | mxa/search-input', function(hooks) {
  ...
  test('it renders', async function(assert) {
    this.set('onInput', () => { /* noop */ });

    await render(hbs`<Mxa::SearchInput @onInput={{this.onInput}} @placeholder="Astro Boy" />`);
    assert.dom('[data-test-mxa-search-input]').exists();
    assert.dom('[data-test-mxa-search-input-label]').exists();
    assert.dom('[data-test-mxa-search-input]').hasAttribute('placeholder', 'Astro Boy');
  });
  ...
  test('it is accessible', async function(assert) {
    ...
    assert.dom('[data-test-mxa-search-input-label]').hasAttribute('for', inputId);
