...
module('Integration | Component | mxa/unbound-input', function (hooks) {
  ...
  test('it displays an accessible input if a label text and an id is provded', async function (assert) {
    ...
    await render(
      hbs`<Mxa::UnboundInput @label={{this.label}} @id="address-field" @onInput={{this.onInput}}/>`
    );
  ...
  test('the disabled input state is accessible', async function (assert) {
    ...
    await render(
      hbs`<Mxa::UnboundInput @onInput={{this.onInput}} @label="Address" @id="address-field" disabled />`
    );
    await a11yAudit();
