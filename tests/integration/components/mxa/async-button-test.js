...
module('Integration | Component | mxa/async-button', function(hooks) {
  ...
  module('loading states', function(hooks) {
    ...
    test('it hides the loading spinner by default', async function(assert) {
      await render(hbs`<Mxa::AsyncButton @onClick={{this.onClick}}>Synth</Mxa::AsyncButton>`);
    ...
    test('it displays the loading spinner', async function(assert) {
      ...
      assert.dom('[data-test-async-button-loading] svg').hasClass('animate-spin');
    });
    ...
    test('it retains element width regardless of loading state', async function(assert) {
      await render(hbs`<Mxa::AsyncButton @onClick={{this.onClick}}>Synthetic Background Music</Mxa::AsyncButton>`);
      ...
      await waitUntil(function() {
        return find('[data-test-async-button]').textContent.includes('Synthetic Background Music');
      }, { timeout: 2000 });
