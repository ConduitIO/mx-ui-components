...
import { render, fillIn, find, click, resetOnerror, setupOnerror } from '@ember/test-helpers';
...
module('Integration | Component | mxa/tags-input', function(hooks) {
  ...
  module('with label block', function(hooks) {
    ...
    test('it renders', async function(assert) {
      ...
      assert.dom('[data-test-tags-input').hasAttribute('placeholder', 'e.g. Florist');
      assert.dom('[data-test-tags-input-label').exists();
      assert.dom('[data-test-tags-input-label-main').includesText('Profession');
      assert.dom('[data-test-tags-input-label-extra').includesText('what you do for a living');
    });
  ...
  module('without label block', function(hooks) {
    hooks.beforeEach(async function(assert) {
      setupOnerror((error) => {
        ...
        assert.ok(error.message.includes(`To render an accessible input element, you must pass at least a <:label-main> block`));
      });
    });
