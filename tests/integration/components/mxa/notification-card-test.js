...
module('Integration | Component | mxa/notification-card', function(hooks) {
  ...
  hooks.beforeEach(function() {
    ...
    this.set('detailedError', { message: 'Oh noes', details: ['pow', 'woops', 'argh'] });
    this.set('spyAction', sinon.spy());
  });

  test('it renders (default/success)', async function(assert) {
    await render(hbs`<Mxa::NotificationCard @isError={{false}} @onDismiss={{this.dismissAction}} @message="This is a success!" />`);

    assert.dom('[data-test-notification-card]').includesText('This is a success');
    assert.dom('[data-test-success-dismiss]').includesText('Dismiss');
  });

  test('it renders (simple error)', async function(assert) {
    await render(hbs`<Mxa::NotificationCard @isError={{true}} @onDismiss={{this.dismissAction}} @message="Oops!" />`);
  ...
  test('it renders (detailed error)', async function(assert) {
    await render(hbs`<Mxa::NotificationCard @onDismiss={{this.dismissAction}} @error={{this.detailedError}} />`);
  ...
  test('it triggers the dismiss action (default/success)', async function(assert) {
    ...
    await render(hbs`<Mxa::NotificationCard @onDismiss={{this.spyAction}} @message="This is a success!" />`);
  ...
  test('it triggers the dismiss action (error)', async function(assert) {
    ...
    await render(hbs`<Mxa::NotificationCard @onDismiss={{this.spyAction}} @error={{this.detailedError}} />`);
